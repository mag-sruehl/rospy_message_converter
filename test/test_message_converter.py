...
class TestMessageConverter(unittest.TestCase):
    ...
    def test_dictionary_with_bytemulti_array(self):
        from std_msgs.msg import ByteMultiArray
        expected_message = ByteMultiArray(data=[1, 2, 3])
        dictionary = {"data": (1, 2, 3)}
        message = message_converter.convert_dictionary_to_ros_message('std_msgs/ByteMultiArray', dictionary)
        expected_message = serialize_deserialize(expected_message)
        print (message.data)
        self.assertEqual(message.data, expected_message.data)
        self.assertEqual(type(message.data), type(expected_message.data))
